...
QStatus SslSocket::PullBytes(void*buf, size_t reqBytes, size_t& actualBytes)
{
    ...
    if (0 > r) {
        ...
        if (0 == err) {
            ...
            status = ER_FAIL;
            QCC_LogError(status, ("BIO_read failed with error=%d", err));
        }
    } else if ((0 == r) && (0 < reqBytes)) {
        status = ER_WOULDBLOCK;
    }
...
QStatus SslSocket::PushBytes(void*buf, size_t numBytes, size_t& numSent)
{
    ...
    int s = BIO_write(bio, buf, numBytes);
    if (0 > s) {
        int err = ERR_get_error();
        if (0 == err) {
            s = 0;
            ...
            status = ER_FAIL;
            QCC_LogError(status, ("BIO_write failed with error=%d", err));
        }
    }
